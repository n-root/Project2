...
windspeed_summary <- exact_ws_data %>%
  left_join(exact_weather_cond_data, by = "time") %>%
...
```{r}
ggplot(windspeed_data, aes(x = time, y = windspeed_10m, color = as.factor(exact_weather_cond_data$weather_description))) + geom_line() + labs(title = "Wind Speed Over Time", x = "Time", y = "Wind Speed (m/s)", color = "Weather Description")
```
...
```{r}
ggplot(windspeed_data %>% left_join(exact_weather_cond_data, by = "time"), aes(x = weather_description, y = windspeed_10m, fill = weather_description)) + geom_boxplot() + labs(title = "Wind Speed Distribution by Weather Description", x = "Weather Description", y = "Wind Speed (m/s)") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```


#### Plot 3: Bar Plot
```{r}
ggplot(exact_ws_data %>% left_join(exact_weather_cond_data, by = "time"), aes(x = weather_description, y = windspeed_10m, fill = weather_description)) + geom_boxplot() + labs(title = "Wind Speed Distribution by Weather Description", x = "Weather Description", y = "Wind Speed (m/s)") + theme(axis.text.x = element_text(angle = 45, hjust =1))
```
...
```{r}
# Collect data
exact_ws_data$hour <- format(exact_ws_data$time, "%H")

# Format data
heatmap_wind_data <- exact_ws_data %>%
  left_join(exact_weather_cond_data, by = "time") %>%
